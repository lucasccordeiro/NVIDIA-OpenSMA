...
| [#4247](https://github.com/esbmc/esbmc/issues/4247) | bundled `<bit>` pointer-to-pointer `bit_cast` overload uses `reinterpret_cast`, rejecting const `From` (residual gap after #4191/#4192) | **fixed** by [#4250](https://github.com/esbmc/esbmc/pull/4250) (merged 2026-05-02) | `stubs/bit` retained pending binary bump |
| [#4248](https://github.com/esbmc/esbmc/issues/4248) | bundled `<span>` does not transitively include `<bit>`; production code relies on that transitive include for `std::bit_cast` | **fixed** by [#4249](https://github.com/esbmc/esbmc/pull/4249) (merged 2026-05-02) | `stubs/span` retained pending binary bump |
