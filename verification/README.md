...
| [#4240](https://github.com/esbmc/esbmc/issues/4240) | `--overflow-check` / `--ub-shift-check` miss signed left-shift result overflow UB under `--std c++20` (ESBMC bitvector model wraps silently; C++23 semantics applied unconditionally) | open | `buffer_to_uint32` F-6 not provable by ESBMC; repro: `esbmc_bug_repros/signed_shift_result_overflow.cpp` |
