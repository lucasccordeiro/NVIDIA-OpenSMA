...
> [esbmc#4448](https://github.com/esbmc/esbmc/issues/4448) — bit_cast
...
> pointer-provenance tracker. Effect: ssif's `i2c_callback →
> smbus_block_*` data path is currently uncovered (Phase 4 reads
> 23/342 = 6.7 % accordingly). In-tree reproducer header at
> `verification/esbmc_bug_repros/bit_cast_member_buffer_bound_loss.cpp`.
> Same family as #4180 part 1.
...
| [#4267](https://github.com/esbmc/esbmc/issues/4267) | `--overflow-check` false-positive misaligned-access on `[[gnu::packed]]` bitfield struct constructor | **fixed** (residual packed-bitfield-constructor case re-filed as #4281, fixed by [#4283](https://github.com/esbmc/esbmc/pull/4283)) | (workaround removed) |
| [#4281](https://github.com/esbmc/esbmc/issues/4281) | `[[gnu::packed]]` bitfield member-initialiser in constructor — false-positive bounds/alignment check | **fixed** by [#4283](https://github.com/esbmc/esbmc/pull/4283) | `--no-align-check` removed from `nsm_f6_system` / `nsm_f7_system` |
