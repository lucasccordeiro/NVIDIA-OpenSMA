...
| `mctp_dispatch` | F-1 reachability: Control SetEpId + validator + router | ✅ CEX on `set_cur_eid(iface∈[2,18))` | — | ✅ (expected FAILED) |
...
| #TBD | `platforms::Control` default-construction crashes ESBMC (`clang_c_adjust_expr.cpp:158` assertion) | open (to be filed) | `mctp_dispatch` harness calls `set_cur_eid()` directly; production link confirmed by code inspection |

## Findings

- **F-1**: `pdk::mctp::platforms::set_cur_eid()` writes to a 2-element
  array indexed by a wire-supplied `interface` without bounds-checking.
  ESBMC proves (`mctp_dispatch`): a Control SetEpId Request with
  `interface ∈ [UsEnd=2, End=18)` passes `Validator::validate()`, then
  `cur_eid.at(interface)` OOBs (counterexample: `iface=2, valid=true`).
  Code inspection confirms `on_set_endpoint_id()` makes this call
  unconditionally (`pdk-mctp-platforms-control.cpp:61`). Production builds
  with `-fno-exceptions` → `abort()` on OOB; SIGABRT confirmed via ESBMC
  ctest gen (see `ctest/f1/`). Fix is one line (mirror `get_cur_eid`).
