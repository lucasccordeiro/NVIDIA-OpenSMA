...
| [#4243](https://github.com/esbmc/esbmc/issues/4243) | bundled `<array>` value-init (`{}`) does not zero-initialise `elems` — two violations: (1) bundled `class array` with `private: elems[N]` is not an aggregate, violating [array.overview]; (2) ESBMC skips the zero-init step of value-initialisation for non-user-provided default constructors ([dcl.init.general]/8), leaving elements as nondet and causing false-positive overflow VCCs on SMA filter accumulators | **open** | `<array>` shim (aggregate `struct`) |
