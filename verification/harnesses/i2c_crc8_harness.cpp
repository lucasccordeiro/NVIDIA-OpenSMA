// ESBMC harness for nv::i2c::crc8 (production: src/nv/i2c/helper.{h,cpp}).
//
// Production helper.cpp also defines `dump()` which calls nv::info() —
// not in scope. Just verifying the two crc8 overloads. The 256-entry
// lookup table is copied verbatim from production.
//
// Phase 1: confirm `LookupTable.at(crc ^ item)` is in-bounds for any
//   uint8_t input (trivially yes — table size 256, byte XOR byte ∈ [0,255]).
// Phase 2: incrementality contract:
//   crc8(0, [a, b]) == crc8(crc8(0, [a]), [b])

#include <cstdint>
#include <array>
#include <span>

extern "C" {
uint8_t  nondet_u8();
unsigned nondet_uint();
}

namespace verif {

// Verbatim from src/nv/i2c/helper.cpp.
static const std::array<uint8_t, 256> LookupTable = {
    ,
};

// Production uses range-for (`for (auto item : data)`); our verification
// <span> shim lacks begin()/end() iterators (we add only what each target
// needs). Index-based loop below is byte-equivalent.
uint8_t crc8(std::span<const uint8_t> data)
{
    uint8_t crc = 0;
    for (unsigned long i = 0; i < data.size(); ++i) {
        crc = LookupTable.at(crc ^ data[i]);
    }
    return crc;
}

uint8_t crc8(uint8_t crc, std::span<const uint8_t> data)
{
    for (unsigned long i = 0; i < data.size(); ++i) {
        crc = LookupTable.at(crc ^ data[i]);
    }
    return crc;
}

}  // namespace verif

namespace {

// Bound the input span; CRC8 is byte-by-byte, properties are length-agnostic.
constexpr unsigned long N = 4;

void f_crc8_simple_total()
{
    uint8_t buf[N];
    for (unsigned i = 0; i < N; ++i) buf[i] = nondet_u8();
    std::span<const uint8_t> s{buf, N};
    (void)verif::crc8(s);
}

void f_crc8_incremental_total()
{
    uint8_t buf[N];
    for (unsigned i = 0; i < N; ++i) buf[i] = nondet_u8();
    std::span<const uint8_t> s{buf, N};
    uint8_t                  seed = nondet_u8();
    (void)verif::crc8(seed, s);
}

void f_crc8_incremental_contract()
{
    // crc8(0, [a, b]) == crc8(crc8(0, [a]), [b])
    uint8_t a = nondet_u8();
    uint8_t b = nondet_u8();
    uint8_t both[2] = {a, b};
    uint8_t one_a[1] = {a};
    uint8_t one_b[1] = {b};

    uint8_t direct  = verif::crc8(std::span<const uint8_t>{both, 2});
    uint8_t partial = verif::crc8(0, std::span<const uint8_t>{one_a, 1});
    uint8_t chained = verif::crc8(partial, std::span<const uint8_t>{one_b, 1});

#ifdef ESBMC_FUNCTIONAL
    __ESBMC_assert(direct == chained, "crc8 incrementality");
#else
    (void)direct;
    (void)chained;
#endif
}

void f_crc8_init_zero_invariant()
{
    // crc8(0, data) must equal crc8(data) for any data.
    uint8_t buf[N];
    for (unsigned i = 0; i < N; ++i) buf[i] = nondet_u8();
    std::span<const uint8_t> s{buf, N};

    uint8_t a = verif::crc8(s);
    uint8_t b = verif::crc8(0, s);

#ifdef ESBMC_FUNCTIONAL
    __ESBMC_assert(a == b, "crc8(s) == crc8(0, s)");
#else
    (void)a;
    (void)b;
#endif
}

}  // namespace

int main()
{
    switch (nondet_uint() % 4) {
        case 0: f_crc8_simple_total(); break;
        case 1: f_crc8_incremental_total(); break;
        case 2: f_crc8_incremental_contract(); break;
        case 3: f_crc8_init_zero_invariant(); break;
    }
    return 0;
}
