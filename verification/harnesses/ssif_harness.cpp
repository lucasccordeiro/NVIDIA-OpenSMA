...
// of scope for this Phase 1 baseline because of esbmc#4448
// (https://github.com/esbmc/esbmc/issues/4448) — ESBMC's pointer-
// provenance tracker loses the parent-buffer bounds across
// `std::bit_cast<Packet*>(_buffer.data())` and reports spurious OOB at
// `&ssif + 1` inside the std::copy_n on `pkt.ipmi_data`. Same family
// as the workaround the c2c_mailbox commit references for
// `pdk-mctp-app-packet.h` (esbmc#4180 part 1). Confirmed reproducible
// after exhausting <bit>, <algorithm> shims and the launder/void-pivot
// variants. Tracked separately as the `ssif_block_data_path`
// follow-up; needs either #4448 to land upstream or a verification-
// only overlay of `ssif.h` that replaces the bit_cast factories with
// reinterpret_cast at struct-decl scope.
//
// Stubs in play:
